...
describe "HL7 ADT^A31 message handling: 'Update person information'" do
  ...
  let(:message) do
    hl7 = <<-HL7
      ...
      PID|1|#{nhs_number}^^^NHS|#{local_patient_id}^^^Dover||#{family_name}^#{given_name}^#{middle_name}^^#{title}||#{dob}|#{sex}||Not Specified|34 Bill \\T\\ Ben Road^SOUTH CROYDON^Surrey^^CR2 0PP^ZZ993CZ^HOME^QAD||x@y.com^EMAIL~tel1^HOME~""^MOBILE|5554443333|NSP||NSP|||||Not Specified|.|DNU||8||NSP|#{died_on}|
