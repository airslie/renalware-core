...
describe "HL7 ADT^A03 message handling: 'Patient Discharge'" do
  ...
  context "when the patient is found in Renalware but has no admission matching the visit_number" do
  ...
  context "when the patient is found in Renalware" do
    ...
    context "when they have an existing admission matching the visit_number" do
      it "updates the admission (TBC?)" do
        ...
        admission.reload
        expect(admission).to have_attributes(visit_number: visit_number)
        expect(admission.hospital_ward.code).to eq("ward")
      end
    end
  end
end
