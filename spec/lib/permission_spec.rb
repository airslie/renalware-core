...
describe Permission, type: :lib do
  describe 'all' do
    ...
    it 'contains system permissions for Renalware Roles' do
    ...
    describe 'abilities for roles' do
      ...
      it 'enables admins to manage specific models' do
        ...
        expect(@admin_permission.models).not_to include(User)
        expect(@admin_permission.models).to include(Drug)
        expect(@admin_permission.models).to include(Patient)
      end

      it 'enables clinicians to manage common models' do
        expect(@clinical_permission.ability).to eq(:manage)
        expect(@clinical_permission.models).to include(Patient)
        expect(@clinical_permission.models).not_to include(Drug)
        expect(@clinical_permission.models).not_to include(Role)
      end

      it 'enables clinicians to view admin models' do
        expect(@clinical_read_permission.ability).to eq(:read)
        expect(@clinical_read_permission.models).to include(Drug)
      end
