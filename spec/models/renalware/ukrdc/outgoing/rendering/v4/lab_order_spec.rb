...
module Renalware
  module UKRDC
    module Outgoing
      describe Rendering::V4::LabOrder do
        ...
        it "outputs PrePost when the observation description has an HD sample type" do
          request = build_pathology_request_with_an_hgb_observation(hd_sample_type: "post")

          actual_xml = format_xml(described_class.new(patient:, request:).xml)

          expect(actual_xml).to include("<PrePost>POST</PrePost>")
        end
      end
    end
  end
