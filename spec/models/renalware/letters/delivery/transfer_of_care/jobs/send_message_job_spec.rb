...
module Renalware::Letters::Delivery::TransferOfCare
  describe Jobs::SendMessageJob do
    ...
    describe "the happy path" do
      it "creates the FHIR XML payload, and sends to the API client, stores the returned msg id" do
        letter.patient.practice.update!(toc_mesh_mailbox_id: "test_mailbox_id")

        allow(BuildPayload).to receive(:call).and_return("PAYLOAD")
        ...
        expect(BuildPayload).to have_received(:call)
    ...
    context "when the gp is not a recipient" do
      it "does not send anything" do
        ...
        expect {
          described_class.new.perform(transmission)
        }.to raise_error(Jobs::SendMessageJob::GPNotInRecipientsError)
      end
    end
    ...
    context "when the gp is a recipient but the practice is nil" do
      it "does not send anything" do
        ...
        expect {
          described_class.new.perform(transmission)
        }.to raise_error(Jobs::SendMessageJob::PatientHasNoPracticeError)
      end
    end
