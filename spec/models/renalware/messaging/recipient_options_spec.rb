...
module Renalware::Messaging
  describe RecipientOptions, type: :model do
    ...
    let(:recipient_for_message_re_patient_a) { create(:messaging_recipient) }
    let(:recipient_for_message_re_patient_b) { create(:messaging_recipient) }
    let(:form) do
      MessageForm.new(
        body: "Content",
        subject: "Subject",
    ...
    describe "#to_h" do
      it "returns a hash of Groups" do
        ...
        expect(users).to include(Renalware::Messaging.cast_recipient(author))
