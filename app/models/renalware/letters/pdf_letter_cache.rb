...
# attr_reader :letter, :folder

# def initialize(letter, folder: nil)
#   @folder = folder || Rails.root.join("private", "letters")
#   @letter = letter
# end
...
# def write(pdf_data)
#   File.open(path, "wb"){ |file| file.write(pdf_data) }
#   letter.update_column(:pdf_cache_key, letter.pdf_cache_key)
# end

# pdf_cache_key is used to generate a unique file path where a pdf copy of the letter will be
# stored once rendered. We use a hybrid cache for PDFs, based on underlying rails caching
# but with some changes. See PdfLetterCache.
# Note we use md5 on the rendered html content - the rendered html including layout and not
# just on the letter body - so that any changes to the layout will trigger invalidate the
# cached version.
# def pdf_cache_key
#   @pdf_cache_key ||= "#{id}-#{Digest::MD5.hexdigest(content)}"
# end
