with serum_samples as (
 ...
 where ST.abbreviation = 'SER' and S.deleted_at is null
 ),
...
distinct on (S.patient_id, BS.sample_type_id)
    ...
    COALESCE((select count(*) from renalware_heroic.biobank_aliquots A where A.sample_id = BS.id and A.deleted_at is null), 0) -
        ...
        where A.sample_id = BS.id  and A.deleted_at is NULL )) as avail_aliquot_count
    ...
    where
        BS.collected_at >= (S.collected_at - interval '15 days') AND  BS.collected_at<= (S.collected_at + interval '15 days') and BS.deleted_at is NULL
