with tmp_events as (
    ...
    FROM
        events
    WHERE ((events.type)::text = 'Renalware::Heroic::Events::Ecg'::text)
...
SELECT
    ...
    case when event_0_id is null then 'Y' else 'N' END as ecg_0_missing,
    case when event_1_id is null and effective_biopsy_date_was_more_than_365_days_ago = true then 'Y' else 'N' END as ecg_1_missing,
    case when event_2_id is null and effective_biopsy_date_was_more_than_1095_days_ago = true then 'Y' else 'N' END as ecg_2_missing
