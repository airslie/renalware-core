...
module Renalware::Heroic
  describe BioBank::AliquotPolicy, type: :policy do
    ...
    context "when the aliquot was created less than 24 hours ago" do
      let(:aliquot) { create(:bio_bank_aliquot, sample: sample, by: user) }

      permissions :destroy? do
        it "permits a super admin" do
          user = create(:user, :super_admin)
          expect(policy).to permit(user, aliquot)
        end
        ...
        it "doesn't permit anyone else" do
          study
          user = create(:user, :clinical)
          expect(policy).not_to permit(user, aliquot)
        end
      end
    end

    context "when the aliquot was created more than 24 hours ago" do
      let(:aliquot) { create(:bio_bank_aliquot, sample: sample, by: user, created_at: 2.days.ago) }
