    ...
    class ExportPatients
      ...
      def call
        ...
        folder_name = within_new_folder do |dir|
          query.all.each do |patient|
            xml_filepath = filepath_for(patient, dir, :xml)
            encrypted_xml_filepath = filepath_for(patient, dir, :encrypted)
            File.open(filepath_for(patient, dir, :xml), "w") do |file|
              file.write("hay") #render_xml_for(patient))
            end
            gpg_encrypt(xml_filepath, encrypted_xml_filepath)
          end
        end
        puts "results in #{folder_name}"
      end
      ...
      def filepath_for(patient, dir, sub_folder)
        ...
        filepath = File.join(dir, sub_folder.to_s, filename)
      end
    end

    ExportPatients.new(**args.to_h).call
  end
end
