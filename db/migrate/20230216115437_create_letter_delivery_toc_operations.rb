class CreateLetterDeliveryTocOperations < ActiveRecord::Migration[6.0]
  def change
    within_renalware_schema do
      enable_extension "pgcrypto" unless extension_enabled?("pgcrypto")

      create_enum :toc_itk3_response_type, %w(
        inf
        bus
        unknown
      )

      create_enum :toc_mesh_message_direction, %w(
        outbound
        inbound
      )

       create_enum :toc_api_action, %w(
        endpointlookup
        handshake
        check_inbox
        download_message
      ...
      create_table :letter_delivery_toc_operations, comment: table_comment do |t|
        t.uuid(
          :uuid,
        ...
        t.enum(
          :direction,
          enum_type: :toc_mesh_message_direction,
          null: false,
          default: :outbound,
          index: true,
        ...
        t.enum(
          :action,
          enum_type: :toc_api_action,
          null: false
        )
        t.references(
          :transmission,
          foreign_key: { to_table: :letter_delivery_toc_transmissions },
          index: true,
        ...
        t.references(
          :parent,
          foreign_key: { to_table: :letter_delivery_toc_operations },
          index: true,
          null: true,
        ...
        t.text(
          ...
          comment: "MESH EPLmailbo/NHS number error code eg EPL-153"
        )
        t.text(
        ...
        t.enum(
          :itk3_response_type,
          enum_type: :toc_itk3_response_type,
          index: true,
          comment: "Incoming messages, where they are an async response to a previously sent " \
                   "message will be of type 'infratstructure' or 'business'"
        )
        t.text(
          :itk3_response_code,
        ...
        t.boolean(
          :itk3_error,
          comment: "true if an ITK3 error was returned in a business or infrastructre reply"
        )
