SELECT p.id,
    ...
    convert_to_float((pa."values" -> 'URE'::text) ->> 'result'::text, null) AS ure,
    ((pa."values" -> 'URE'::text) ->> 'observed_at'::text)::date AS ure_date,
    convert_to_float((pa."values" -> 'CRE'::text) ->> 'result'::text, null) AS cre,
    ((pa."values" -> 'CRE'::text) ->> 'observed_at'::text)::date AS cre_date,
    convert_to_float((pa."values" -> 'EGFR'::text) ->> 'result'::text, null) AS egfr_on,
