-- Create a view to back Letters::Transports::Mesh::Letter.
-- Tabulates and simplifies some complex relational data relating to a letter MESH transmission
-- and its send + download (inf) and download (bus) operations.
select
  ll.id,
  ll.approved_at,
  ll.completed_at,
...
inner join users author on author.id = ll.created_by_id
