<?php
//----Mon 13 Jan 2014----path search fixes
$debug=false;
...
$searchtables="renalware.patientdata p JOIN renalware.renaldata r ON patzid=renalzid LEFT JOIN renalware.esddata ON patzid=esdzid";

if ($_POST["currsite"] or $_POST["currsched"] or $_POST["currslot"]) {
    //add table
    $searchtables.=" JOIN renalware.hdpatdata ON patzid=hdpatzid";
}
...
if ($post_problem) {
    //add table
    $searchtables.=" JOIN renalware.problemdata ON patzid=probzid";
    $fields.=",problem";
    //simple OPER
    $probstring = strtolower($post_problem) ;
    $proboper=$sqlopers[$post_problemoper];
    switch ($proboper)
        {
       case 'matches':
           $where.=" AND LOWER(problem) = '$probstring'";
           break;
       case 'contains':
           $where.=" AND LOWER(problem) LIKE '%$probstring%'";
           break;
       case 'starts with':
           $where.=" AND LOWER(problem) LIKE '$probstring%'";
           break;
...
if ($post_drugname) {
    //add table
    $searchtables.=" JOIN renalware.medsdata ON patzid=medzid";
    $fields.=",drugname";
    //simple OPER
    $medstring = strtolower($post_drugname) ;
    $medoper=$sqlopers[$post_drugnameoper];
    switch ($medoper)
        {
       case 'matches':
           $where.=" AND LOWER(drugname) = '$medstring'";
           break;
       case 'contains':
           $where.=" AND LOWER(drugname) LIKE '%$medstring%'";
           break;
       case 'starts with':
           $where.=" AND LOWER(drugname) LIKE '$medstring%'";
           break;
        } //end switch sqlop
		if ($post_drugsearchtype=="Current") {
			$where.=" AND termflag=0";
		}
    }
