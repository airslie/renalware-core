...
$ufn=$_POST['ufn'];
$pfn=$_POST['pfn'];
if ($_POST["$ufn"] && $_POST["$pfn"]) {
    $u = $mysqli->real_escape_string($_POST["$ufn"]);
    $p = $mysqli->real_escape_string($_POST["$pfn"]);
	$sql = "SELECT uid, user, adminflag, decryptflag, editflag, printflag, bedmanagerflag, authorflag,pathflag, sitecode, passmodifstamp, DATEDIFF(NOW(),passmodifstamp) as passwordage from userdata where user='$u' AND pass=PASSWORD('$p') LIMIT 1";
	...
	if ($row)
		...
		if ($p=='password' or !$passmodifstamp or $row["passwordage"]>$pwexpiredays)
			{
