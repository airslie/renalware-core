...
foreach ($form_data as $fldname => $fldtypedata) {
	...
	switch ($fldtype) {
		case 't':
			...
			echo '<input type="text" name="'.$fldname.'" id="'.$fldname.'" size="'.$fldsize.'" value="'.${$fldname}.'" />';
			break;
		case 'd':
			echo '<input type="text" name="'.$fldname.'" id="'.$fldname.'" size="12" value="'.${$fldname}.'"/>';
			break;
		case 'a':
			$rows_cols=explode('x', substr($fldtypedata,1));
			$rows=$rows_cols[0];
			$cols=$rows_cols[1];
			echo '<textarea name="'.$fldname.'" rows="'.$rows.'" cols="'.$cols.'">'.${$fldname}.'</textarea>';
			break;
		case '1':
		...
		$options=${$fldname ."_options"};
			...
			echo ${$fldname}?'<option value="'.${$fldname}.'">'.${$fldname}.'</option>':'<option value="">Select...</option>';
